...
export const MilestoneMarker: React.FC<MilestoneMarkerProps> = ({
  ...
  return (
    <>
      ...
      >
        {/* Icono de dependencia + etiqueta + acciones */}
        <div className="relative flex items-center h-full cursor-grab active:cursor-grabbing">
          {onLinkStart && (
            <button
              ...
              className={`w-4 h-4 rounded-full hover:bg-cyan-600 text-slate-300 flex items-center justify-center cursor-grab active:cursor-grabbing ${
                isLinkSource ? "bg-cyan-600 ring-2 ring-cyan-400" : "bg-slate-700"
              }`}
          ...
          {editing ? (
            ...
            <div
              className="ml-2 px-2 py-0.5 rounded bg-slate-900/90 border border-slate-700 text-xs text-orange-100 font-medium whitespace-nowrap cursor-grab active:cursor-grabbing"
              onDoubleClick={(e) => {
                e.stopPropagation();
                setDraft(milestone.label);
                setEditing(true);
